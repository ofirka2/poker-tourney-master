...
const MultiStepTournamentForm: React.FC<MultiStepTournamentFormProps> = ({
  ...
  const generateBlindLevels = () => {
    ...
    if (!chipsetArray.length) {
      ...
      return generateDefaultBlindStructure();
    }
    ...
    return generateDynamicBlinds(
      ...
      desiredDurationMinutes,
      {
        ...
        anteStartLevel: formData.includeAnte ? 4 : 999, // Start antes at level 4 if enabled, otherwise disable
  ...
  /**
   * Generates a default blind structure as a fallback.
   * @returns {Array<object>} An array of blind level objects.
   */
  const generateDefaultBlindStructure = () => {
    ...
    // Start with the calculated small and big blinds
    let smallBlind = formData.smallBlind;
    ...
    for (let i = 1; i <= numLevels; i++) {
      ...
      if (i > 1 && i % 4 === 0) {
        ...
        // Increase blinds for next level (simple progression)
        if (i < numLevels) { // Only calculate for the next level if it exists
           // Simple doubling every 2 levels
           if (i % 2 === 0) {
             smallBlind = Math.round(smallBlind * 1.5); // Example increase
             bigBlind = smallBlind * 2;
           }
           // Simple ante increase (example)
           if (formData.includeAnte && i >= 3) { // Increase ante after level 3
  ...
  // Effect to update initial blinds display when startingStack changes (if not manually set)
  // This useEffect is less critical now that the main useEffect handles stack/blind calculation
  // based on format/chipset/duration changes. You might remove this if it's redundant.
  // useEffect(() => {
  //    // Update smallBlind/bigBlind display when startingStack changes,
  //    // but only if they haven't been manually overridden or are linked to stack.
  //    // This logic is complex and might be better handled by the main calculation effect.
  // }, [formData.startingStack]);
