...
const TournamentSetup: React.FC<TournamentSetupProps> = ({ tournamentId }) => {
  ...
  const {
      ...
      chipsetValues, // This is still derived inside the hook, not directly set here
      initialChips,
      setInitialChips,
      ...
      smallBlind, // This is derived from initialChips, etc. in the hook
      bigBlind,   // This is derived from initialChips, etc. in the hook
      levels,
      setLevels,
      maxRebuys,
      setMaxRebuys,
      ...
      initialAllowRebuy: settings.allowRebuy ?? true, // Pass this so hook can calculate rebuy/addon factors
      initialMaxRebuys: settings.maxRebuys ?? 0, // Pass this to the hook
      ...
      if (tournamentId) {
        ...
        try {
          ...
          if (data) {
            ...
            setPlayerCount(data.no_of_players || 9); // Corrected from data.player_count
            setDurationHours(data.desired_duration ? data.desired_duration / 60 : 4); // Corrected from data.duration_hours
  ...
  const saveSettings = useCallback(async () => {
    ...
    const updatedSettings = {
      ...
      playerCount,
      desiredDuration: durationHours, // Corrected to use durationHours
      format: tournamentFormat, // Corrected to use format
      chipset,
    ...
    if (tournamentId) {
      ...
      try {
        const tournamentData = {
          ...
          no_of_players: playerCount, // Corrected to no_of_players
          desired_duration: durationHours * 60, // Store in minutes in DB
