...
import TableAssignment from "@/components/tables/TableAssignment";
...
const Tables = () => {
  ...
  const loadTournamentData = useCallback(async (id: string) => {
    if (!id) {
      toast.info("No tournament ID specified in URL for tables.");
    ...
    setLoading(true);
    console.log(`Tables: Attempting to load tournament with ID: ${id}`);
    ...
    try {
      ...
      if (error) {
        console.error('Supabase error loading tournament for Tables:', error);
      ...
      if (data) {
        ...
        if (playersError) {
          console.error('Error fetching players for tables:', playersError);
        ...
        dispatch({
          ...
          payload: {
            ...
            isRunning: data.status === 'In Progress',
        ...
        toast.success(`Loaded tournament for table assignment: ${data.name}`);
      } else {
        console.warn(`Tables: No tournament found with ID: ${id}`);
        toast.error(`Tournament with ID ${id} not found.`);
        dispatch({ type: 'RESET_TOURNAMENT' });
      }
    } catch (error) {
      console.error('Error loading tournament for tables:', error);
  ...
  useEffect(() => {
    if (tournamentId && tournamentId !== processedTournamentId) {
      loadTournamentData(tournamentId);
        ...
        if(state.id) {
            dispatch({ type: 'RESET_TOURNAMENT' });
            toast.info("Navigated to generic tables. Create or select a tournament.");
        }
    ...
    return (
      <Layout>
        <div className="flex justify-center items-center h-64">
          ...
          <p className="ml-4 text-muted-foreground">Loading tournament tables...</p>
        </div>
      </Layout>
    );
  }
      ...
      <Layout>
        <div className="text-center py-10">
          ...
          <p className="text-muted-foreground mb-4">
            Please select a tournament from the homepage to manage table assignments.
          </p>
        </div>
      </Layout>
    );
  ...
  // Show table assignment when everything is loaded
  ...
  return (
    <Layout>
      <div className="flex justify-center items-center h-64">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
        <p className="ml-4 text-muted-foreground">Preparing table assignment...</p>
      </div>
    </Layout>
  );
};
